 ...
 *      halfExpandedOffset
 *      	fitToContents==false,展开一半的偏移值=父布局高度*(1-halfExpandedRatio)
 *      	对应状态:STATE_HALF_EXPANDED
 *      halfExpandedRatio
 *      	阈值,STATE_HALF_EXPANDED状态时View高度占父布局的百分比,fitToContents=父布局高度*(1-halfExpandedRatio)
 *
 * hideable:是否隐藏
 *    true:下拉时可能隐藏
 *      skipCollapsed:跳过折叠状态,下拉时直接隐藏
 *    false:无法下拉隐藏
 *    相关参数:
 *      SIGNIFICANT_VEL_THRESHOLD
 *        阈值:垂直滑动的速度(每秒划过的像素值),超过该速度阈值隐藏
 *      HIDE_THRESHOLD
 *        阈值:半分比距离,释放手势时,如果该位置在折叠区域下,且下拉像素点超过HIDE_THRESHOLD百分比距离,则隐藏
 *      HIDE_FRICTION
 *        移动惯性时间(单位:秒):释放手势,此时的y坐标加上移动速度*HIDE_FRICTION,为推测的停止y坐标
 *
 * View的背景
 *    shapeThemingEnabled
 *      是否在xml中提供了shapeAppearance属性
 *    isShapeExpanded
 *      初始化时,是否是展开状态
 *    interpolatorAnimator
 *      差值器动画,为materialShapeDrawable设置差值(0,1)
 *    shapeAppearanceModelDefault
 *      materialShapeDrawable的shape
...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
   ...
   *
   * 根据父布局的宽度和16:9的比例,计算对应高度为子布局折叠时的高度
   *
  ...
  /** 设置无障碍焦点,如果为false,默认播报内容,如果为true,关闭无障碍焦点,不会播报View和子布局的内容 */
  private boolean updateImportantForAccessibilityOnSiblings = false;

  /** 最大y垂直移动速度,单位:1000ms移动的像素值 */
  private float maximumVelocity;

  /** 折叠时View的高度 */
  private int peekHeight;

  /** 是否自动生成折叠高度,此时peekHeight == PEEK_HEIGHT_AUTO */
  ...
  /** 释放手势,View动画移动到状态的Runnable,不断执行动画直至到达状态未知 */
  private SettleRunnable settleRunnable = null;

  /** 差值动画,为MaterialShapeDrawable服务,展开时差值为0,折叠时为1 */
  ...
  /** 是否忽略Events,根据该标志判断是否要消费或处理事件 */
  private boolean ignoreEvents;

  /** 上一个滚动的y距离 */
  private int lastNestedScrollDy;

  /** 是否正在滚动,用于避免多次onStopNestedScroll回调的执行 */
